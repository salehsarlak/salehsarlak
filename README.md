<div align="center">
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6366f1,50:8b5cf6,100:ec4899&height=6&section=header" width="100%">
<h1>Hi, I'm Saleh Sarlak 👋</h1>
<h3>Web Designer · WordPress Developer · Founder of Tarhfam</h3>
<a href="https://linkedin.com/in/salehsarlak">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>
<a href="mailto:sarlaksaleh7@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white">
</a>
<a href="https://tarhfam.ir">
  <img src="https://img.shields.io/badge/Website-6366f1?style=for-the-badge&logo=google-chrome&logoColor=white">
</a>
<img src="https://komarev.com/ghpvc/?username=salehsarlak&style=for-the-badge&color=6366f1&label=PROFILE+VIEWS">
<br/>
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&weight=600&size=22&duration=3000&pause=800&color=A78BFA&center=true&vCenter=true&width=700&lines=Web+Designer+%26+WordPress+Developer;UI%2FUX+%E2%80%A2+Elementor+Pro+%E2%80%A2+WooCommerce;Founder+of+Tarhfam;Crafting+clean+%26+modern+experiences" alt="Typing SVG">
</a>
</div>
<br>
## 🖥️ who am I
<div align="center">
  <img src="./assets/whoami-ironman.svg" alt="whoami" width="100%">
</div>
<br>
## 🛠️ Tech Stack
<div align="center">
  <img src="./assets/techstack.svg" alt="tech stack" width="100%">
</div>
<br>
## 💼 Experience
<div align="center">
  <img src="./assets/experience-timeline.svg" alt="experience timeline" width="100%">
</div>
<br>
<details open>
<summary><b>🟡 Founder & Lead Designer — Tarhfam</b> &nbsp;<sub>2024 – Present</sub></summary>
<br>
Running Tarhfam — a digital agency focused on high-performance web design, WordPress development, SEO, and digital solutions.
<code>WordPress</code> <code>Php</code> <code>Java Script</code> <code>Claude</code> <code>SEO</code>
</details>
<details>
<summary><b>🔵 Web Designer & WordPress Developer — Freelance</b> &nbsp;<sub>2023 – Present</sub></summary>
<br>
Designing & developing high-performance WordPress websites — focusing on UI/UX, SEO, speed, and scalable solutions for businesses.
<code>UI/UX</code> <code>WordPress</code> <code>Php</code> <code>EJava Script</code> <code>Sql</code>
</details>
<br>

<!-- 10px فاصله بین هدر و خط زیرین -->
<h2 style="margin-bottom: 10px;">🎨 Featured Projects</h2>
<hr style="margin-top: 0px;">

### 📚 [Bookivery](https://bookivery.ir)
Custom online bookstore with city-based product availability, seller management, inventory, ordering, and delivery logic.
`WordPress` · `PHP` · `JavaScript`
---
### 📖 [Maz Isfahan](https://mazesfahan.ir)
Professional online bookstore focused on product presentation, e-commerce functionality, and a clean user experience.
`WordPress` · `PHP` · `JavaScript`
---
### 👟 [Patak Shoes](https://patakshoes.ir)
E-commerce website for showcasing and selling shoes and fashion products with a focused shopping experience.
`WordPress` · `PHP` · `JavaScript`
---
### 🏥 [Aman Med Iran](https://amanmediran.com)
Medical tourism website designed to present healthcare services and build trust through a professional digital experience.
`WordPress` · `PHP` · `JavaScript`
<br>

<!-- 10px فاصله بین هدر و خط زیرین -->
<h2 style="margin-bottom: 10px;">📊 GitHub Analytics</h2>
<hr style="margin-top: 0px;">

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=salehsarlak&theme=tokyonight&hide_border=true&border_radius=12&short_numbers=true"/>
  <br>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=salehsarlak&theme=tokyo-night&hide_border=true&area=true"/>
</div>
<br>

<!-- 10px فاصله بین هدر و خط زیرین -->
<h2 style="margin-bottom: 10px;">🧊 3D Contribution Graph</h2>
<hr style="margin-top: 0px;">

<div align="center">
<img src="./profile-3d-contrib/profile-season-animate.svg" alt="3D contribution graph">
</div>
<br>

<!-- 10px فاصله بین هدر و خط زیرین -->
<h2 style="margin-bottom: 10px;">🐍 Contribution Snake</h2>
<hr style="margin-top: 0px;">

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/salehsarlak/salehsarlak/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/salehsarlak/salehsarlak/output/github-contribution-grid-snake.svg">
    <img alt="github contribution snake" src="https://raw.githubusercontent.com/salehsarlak/salehsarlak/output/github-contribution-grid-snake.svg" width="100%">
  </picture>
</div>
<br>
<div align="center">
📫 **Reach me:** [sarlaksaleh7@gmail.com](mailto:sarlaksaleh7@gmail.com) · [Tarhfam](https://tarhfam.ir) · [LinkedIn](https://linkedin.com/in/salehsarlak)
<br>
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:ec4899,50:8b5cf6,100:6366f1&height=6&section=footer" width="100%">
</div>
